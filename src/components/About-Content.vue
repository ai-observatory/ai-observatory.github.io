<template>
  <div>
    <h1>About</h1>

    <p>
      This project was provoked by the marked difference between the progressive
      and hyperbolic discourse on how digital technologies and &lsquo;Artificial
      Intelligence&rsquo; could solve complex problems in governance, and the
      experiences of the people using and being affected by these technologies
      in practice. This is an attempt to both critically analyse the claims of
      AI and Automated Decision Making Systems used in consequential areas, as
      well as inspire greater interrogation of, and participation in,
      conversations about how these systems should be developed and
      governed.&nbsp;
    </p>
    <div>
      <b-img
        center
        style="width: 80%; height: 660px; object-fit: cover"
        src="../../public/pimg1.png"
        fluid
        alt="ml and big data walk into a bar"
      ></b-img>
    </div>
    <p>
      The &lsquo;AI Observatory&rsquo;, is intended to be a participatory
      effort, to be built upon by individuals and communities impacted by ADMS
      in India. It is intended to serve as a larger effort of counter-mapping
      and interrogating the opaque and unaccountable ways in which these systems
      are deployed and used within government institutions in India.&nbsp;
    </p>
    <p>
      Please email me at divij [dot] joshi [at] gmail.com if you have a story or
      experience about your interaction with ADMS in India, or if you have
      resources and documentation to add to the observatory.
    </p>
    <p><strong>Acknowledgements</strong></p>
    <p>
      This project was funded and supported by the Mozilla Foundation, and was
      undertaken during my time as a Mozilla Tech Policy Fellow for 2019-20. My
      work was wonderfully supported by the team at the Mozilla Foundation, and
      particularly by Janice Wait, who provided guidance and encouragement
      throughout the course of this work.
    </p>
    <p>
      I would like to thank the wonderful community of lawyers, activists,
      journalists and technologists for the inspiring and critical conversations
      that have fuelled my thinking about this subject. In particular, I would
      like to thank all the participants in the
      <a
        href="https://www.medianama.com/2020/05/223-announcement-online-event-on-algorithmic-accountability-in-india-jun-4-2020-ad/"
        >Workshop on Algorithmic Accountability in India</a
      >, organised in June, 2020, with Medianama, which provided crucial
      direction and insight in building this project.&nbsp;
    </p>
    <p>
      I am especially grateful to Siddharth Sonkar for his excellent research
      assistance and his inputs on various elements of this project.
    </p>
    <p><strong>Team</strong></p>
    <p>
      Divij Joshi is a lawyer and a researcher studying the intersection of law,
      technology and society. This project was undertaken in his capacity as a
      Mozilla Tech Policy Fellow working on Trustworthy AI, in 2019-20. You can
      read more about his work <a href="https://divij.me/">here</a>.&nbsp;
    </p>
    <p><strong>Design Beku&nbsp;</strong></p>
    <p>
      <a href="https://designbeku.in/">Design Beku</a> strives to make design
      and technology more locally rooted and contextually relevant. We are a
      feminist collective that places an ethics of care at the heart of digital
      and design practice.
    </p>
    <p><strong>Padmini Ray Murray (project lead)&nbsp;</strong></p>
    <p>
      Padmini is an independent researcher and founder of Design Beku, and cares
      deeply about community infrastructures, digital rights and feminism. Find
      her at @praymurray on Twitter.
    </p>

    <p><strong>Paulanthony George (illustrations)</strong></p>
    <p>
      Paul aka
      <a href="https://www.instagram.com/memewhile.mp3/">@memewhile.mp3</a> does
      research on internet memes, spreadable media, online behaviours, and draws
      comics as a part of this research, while writing bios in the third person,
      taking care of cats and gardening.
    </p>
    <p><strong>Pratyush Raman (fullstack development)</strong></p>
    <p>
      Pratyush <a href="https://www.instagram.com/prtyshrmn/">(prtyshrmn)</a> is
      a teaching-artist, engaging in the confluence of art and technology. He
      cooks food and makes music for his soul, keeping aspirations of being a
      polymath.
    </p>
    <p><strong>Senjuti Sangia (typesetting)</strong></p>
    <p>
      Senjuti is a fairly recent and a very perplexed design graduate. In the
      immediate future, she hopes to compound feminist theories and elements of
      theatre.
    </p>
    <p><strong>Varun Kurtkoti (animation)</strong></p>
    <p>
      Varun is a filmmaker and animator, primarily working in the areas of
      individual memory in relation to the collective, the politics of identity,
      and oral cultures.
    </p>

    <p>
      <strong>
        <a href="https://github.com/ai-observatory/ai-observatory.github.io">
          GitHub link
        </a>
        for the website</strong
      >
    </p>
    <p>Supported by:</p>

    <div>
      <b-img
        left
        style="width: 20%"
        src="../../public/logo1.png"
        fluid
        alt="Mozilla Logo"
      ></b-img>
    </div>
  </div>
</template>

<script></script>

<style lang="scss" scoped></style>
